...
#include "packages/framebuf/framebuf.h"
#include "packages/mbedfont/mbedfont.h"

namespace k {
class SimpleCharacterRenderer {
 public:
  SimpleCharacterRenderer(FrameBuffer* buf) : buf_{buf} {}

  const FrameBuffer& frameBuffer() const { return *buf_; }

  void drawCharacter(char c, size_t x, size_t y, uint32_t bgcolor,
                     uint32_t fgcolor) {
    if (c < 0)
      return;
...
class SimpleTextOutput {
 ...
 protected:
  ...
  void drawChar(char c) {
    renderer_->drawCharacter(c, col_ * kFont8x8Width, row_ * kFont8x8Height,
