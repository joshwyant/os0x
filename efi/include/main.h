...
#define Warning(msg, ...) \
  Log(WarningLevel, DEBUGPREFIX(Warning) msg, __VA_ARGS__)
#define Debug(msg, ...) Log(DebugLevel, msg, __VA_ARGS__)
#define Trace(msg, ...) Log(TraceLevel, msg, __VA_ARGS__)

#define ErrorLine(msg, ...) \
  LogLine(ErrorLevel, DEBUGPREFIX(Error) msg, __VA_ARGS__)
#define InfoLine(msg, ...) LogLine(InfoLevel, msg, __VA_ARGS__)
#define WarningLine(msg, ...) \
  LogLine(WarningLevel, DEBUGPREFIX(Warning) msg, __VA_ARGS__)
...
#define TRYWRAPS(tuple, msg, ...) \
  TRYWRAPFNS(uefi_call_wrapper tuple, msg, __VA_ARGS__)
...
#define TRYWRAPFNS(fn, msg, ...) \
  TRYWRAPFN(fn, msg ": %r", __VA_ARGS__ __VA_OPT__(, ) status)
...
// #define memset(a, c, b) uefi_call_wrapper(BS->SetMem, 3, a, b, c)
...
EFI_STATUS measure_kernel(const void* elf_data, size_t elf_size,
                          kernel_image_t* out);
...
EFI_STATUS map_new_pages(page_virtual_address_t virt_addr,
                         ...
                         PageAttributes attr, int pagesi,
                         page_table_physical_ptr_t pageTable);
